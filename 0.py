...
lam='[1;36m'
ntldev157211=""
ntl=trang+'~'+red+'['+vang+'⟨⟩'+red+'] '+trang+'➩ '+lam
...
def get_uptime(domain):
    try:
        ...
        print(f"{ntl} Đã xảy ra lỗi khi kiểm tra uptime: {e}")
        return None
...
def scrape_domains():
    ...
    try:
        ...
        print(f"{ntl} Đã xảy ra lỗi trong quá trình thu thập tên miền: {e}")
    return list(domains)
...
if __name__ == "__main__":
    clear()
    banner()
    output_file = input(f"\n{ntl} Nhập tên file .txt để lưu kết quả (ví dụ: mail-reg-clsm.txt): ")
    num_domains = int(input(f"{ntl} Nhập số lượng tên miền cần thu thập và kiểm tra uptime: "))
    max_uptime = int(input(f"{ntl} Nhập số ngày uptime tối đa để lưu: "))
    num_threads = int(input(f"{ntl} Nhập số luồng để chạy: "))
    attempt = 1
    saved_domains = set()
    total_domains = 0
    ...
    clear()
    banner()
    print(f"\n{ntl} Ấn Tổ Hợp Phím Ctrl + C Để Dừng Tool")
    print(f"{ntl} Bạn đang chạy {num_threads} luồng (phân chia giữa thu thập và kiểm tra uptime)...")

    try:
        while True:
            print(f"{ntl} Đang thu thập và kiểm tra tên miền Lần {attempt}...")
            ...
            if not all_domains:
                print(f"{ntl} Không thu thập được tên miền nào.")
                break
            ...
            attempt += 1
            print(f"{ntl} Đã hết tên miền thu thập trong vòng này. Tiến hành thu thập tiếp lần {attempt}...")
            time.sleep(2)
    except KeyboardInterrupt:
        print(f"{ntl} Người dùng đã dừng chương trình.")
    except Exception as e:
        print(f"{ntl} Đã xảy ra lỗi không xác định: {e}")
    finally:
        print(f"{ntl} Tạm Biệt!")
